...
def _gen_genre_query(corpus: MovieLensCorpus, rng: random.Random) -> QuerySpec | None:
    doc = rng.choice(corpus.documents)
    if not doc.genres:
        return None
    genre = rng.choice(doc.genres)
    label = corpus.genre_labels[genre]
    return QuerySpec(f"{FEATURE_GENRE}:{genre}", f"genre={label}")


def _gen_genre_and_year_query(
...
def _gen_genre_or_genre_query(
    corpus: MovieLensCorpus,
    rng: random.Random,
    docs_with_multi_genre: list,
) -> QuerySpec | None:
...
def _gen_genre_not_query(
    corpus: MovieLensCorpus, rng: random.Random
) -> QuerySpec | None:
    doc = rng.choice(corpus.documents)
    if not doc.genres:
        return None
...
def _gen_tag_and_genre_query(
    ...
    return QuerySpec(
        ...
        (f"tag={corpus.tag_labels[tag]} AND genre={corpus.genre_labels[genre]}"),
    )
...
def _generate_query_specs(
    ...
    *,
    ...
    while len(queries) < num_queries:
        ...
        if choice == "genre":
            result = _gen_genre_query(corpus, rng)
        elif choice == "genre_and_year":
            result = _gen_genre_and_year_query(corpus, rng, docs_with_year)
        elif choice == "genre_or_genre":
            result = _gen_genre_or_genre_query(corpus, rng, docs_with_multi_genre)
        elif choice == "genre_not":
            result = _gen_genre_not_query(corpus, rng)
        else:
            result = _gen_tag_and_genre_query(corpus, rng, docs_with_tags)
...
def main(
    ...
    *,
    ...
    plans_data = plans_data.to(device)
    plans_offsets = plans_offsets.to(device)
